...
public sealed class PublishAsyncTests : LitterServiceTestsBase
{
    ...
    public async Task Publish_WhenLitterNotFound_ThrowsNotFoundException()
    {
        ...
        await Assert.ThrowsAsync<NotFoundException>(() => svc.PublishAsync(LitterId, BreederId));
    }
    ...
    public async Task Publish_WhenBreederIsNotOwner_ThrowsForbiddenException()
    {
        ...
        await Assert.ThrowsAsync<ForbiddenException>(() => svc.PublishAsync(LitterId, BreederId));
    }
    ...
    )
    {
        ...
        await Assert.ThrowsAsync<DomainException>(() => svc.PublishAsync(LitterId, BreederId));
    }

    [Fact]
    public async Task Publish_WhenBenefitNotFound_ThrowsNotFoundException()
    {
        var (svc, litterRepo, benefitRepo, _, _, _) = CreateSut();
    ...
    {
        var (svc, litterRepo, benefitRepo, auditRepo, uow, _) = CreateSut();
        ...
        var ex = await Assert.ThrowsAsync<DomainException>(
            () => svc.PublishAsync(LitterId, BreederId)
        );
    ...
    {
        var (svc, litterRepo, benefitRepo, auditRepo, uow, notifications) = CreateSut();
        ...
        await svc.PublishAsync(LitterId, BreederId);
